...
if (BITSPERPIXEL == 1) begin //If 1 bit per pixel, we use 2 defined colors
	...
	xpm_memory_sprom #(
		...
		/// Task 3: make sure you placed the generated lut in the correct directory.
		.MEMORY_INIT_FILE("lut.mem"),     // String
...
always_ff @(posedge clk_pixel or negedge arstn) begin
    if(!arstn) begin
        ...
        if(hor_cnt == hor_sync_cycles-1) begin
            hor_cnt <= 0;
            ver_cnt <= (ver_cnt==ver_sync_cycles-1) ? 0 : ver_cnt + 1;
        end else hor_cnt <= hor_cnt + 1;
    end
end
...
xpm_memory_sprom #(
	...
	.CASCADE_HEIGHT(0),            // DECIMAL
	.ECC_MODE("no_ecc"),           // String
	.MEMORY_INIT_FILE("text.mem"),     // String
	.MEMORY_INIT_PARAM("0"),       // String
	.MEMORY_OPTIMIZATION("true"),  // String
	.MEMORY_PRIMITIVE("auto"),     // String
